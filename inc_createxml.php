...
$sql = "SELECT * FROM t_files WHERE record_id = ".$_GET['id'].";";
$erg = $db->query($sql);
...
$sql = "SELECT * FROM t_metadata WHERE record_id = ".$_GET['id'].";";
$erg = $db->query($sql);
	
#Schreiben der metadaten-informationen	
while($row=$erg->fetch(PDO::FETCH_ASSOC)){

	$element = $record->appendChild($xml->createElement('dc:'.$row['dc_element'],$row['value']));
